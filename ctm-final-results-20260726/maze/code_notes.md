...
- 任务脚本：`scripts/run_ctm_maze_analysis.py`
...
Maze 任务存在 `S_action` 和 `q_proj(S_action)`。query 是从记录的 `S_action` 通过训练好的 `q_proj` 重新计算得到的；没有改动模型权重或官方 forward 逻辑。

## 可视化文件

- `result.gif`：每个 tick 的迷宫输出/路线变化。
- `query.gif` / `query_overview.png`：query 随 tick 的变化。
- `synchronization_triptych.gif`：`S_action`、`S_out`、活跃神经元 `S_full` 的联合可视化。

## 本次最小代码改动

- 新增/扩展了 `scripts/run_ctm_maze_analysis.py`，用于保存原始 sync/query/S_full 文件和 GIF。
- 官方模型文件没有改动。
